...
696966
Note
In the first test case, we have to perform at least one operation, as a2=3>2. We can perform the operation [1,3,4]→[1,2–,3,4] (the newly inserted element is underlined), now the condition is satisfied.
...
void solve() {
	...
	foi(n) {
		cin >> a[i];
		(a[i] > i + 1 + cnt) ? cnt += (a[i] - i - 1 - cnt) : cnt = cnt;
	}
	cout << cnt;
}
